...
int myfilter(int x)
{
    ...
    for (i=1; i<NT; i++){
        part_sum = sx[NT-1+i] + sx[NT-1-i];

        //optimizing the area, by truncating the sums to 6 bits and the coefficients to 5 bits
		y += ((part_sum >> (NB/2-1)) * (b[NT-1-i] >> (NB/2-1)));
    }
    //add the last weighted term, truncating the term x(n-4) to 5 bits and the coefficient to 5 bits
    y+= ((sx[NT-1] >> (NB/2-1)) * (b[NT-1] >> (NB-1)));
